...
void ZsInstance::ReleaseInstance()
{
    if (_instance != nullptr)
    {
        QMutexLocker locker(&m_instanceMutex);
...
void ZsInstance::NetRequest(const QString &id, int type, QString url, QVector<ZS_NET_REQ_INFO> reqHeader, QByteArray reqData)
{
    ZsNetThread* m_pNetMgr = new ZsNetThread;
    m_pNetMgr->setParam(id, type, url, reqHeader, reqData);
    connect(m_pNetMgr, &ZsNetThread::netReqFinish, this, &ZsInstance::slot_netReqFinish);
    connect(m_pNetMgr, &ZsNetThread::netReqProgress, this, &ZsInstance::slot_netReqProgress);
    m_pNetMgr->start();
...
void ZsInstance::slot_netReqProgress(qint64 bytesReceived, qint64 bytesTotal)
{
    emit sig_netReqProgress(bytesReceived, bytesTotal);
}
